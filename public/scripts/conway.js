...
class Conway {
  constructor(rows, cols, pixelSize, interRoundDelay, initialChanceOfLife) {
    ...
    // Inform each pixel who it's neighbors are
  ...
  */
  setRandomPixelColors() {
    let baseHue = util.randomInteger(1, 360);
    let complementaryHue = (baseHue + util.randomInteger(90, 270) % 360);
  ...
  */
  registerMouseListeners() {
    util.bindMultipleEventListener(this.canvas, ['mousemove', 'touchmove'], (e) =>  {
      e.preventDefault();
      if(this.mouseIsDown) {
        let x, y;
        if(e.touches) {
    ...
    util.bindMultipleEventListener(this.canvas, ['mousedown', 'touchstart'], (e) => {
      e.preventDefault();
      let rect = e.target.getBoundingClientRect();
      let x, y;
      if(e.touches) {
    ...
    util.bindMultipleEventListener(this.canvas, ['mouseup', 'touchend'], (e) => {
      e.preventDefault();
      this.mouseIsDown = false;
    });
  }
